package orichalcum.query  
{
	...
	public class NextUntilTest 
	{
		...
		[Test]
		public function nextUntilOfParentlessEmptySetIsEmpty():void
		{
			assertThat($(new Sprite).nextUntil(Point).length, equalTo(0));
		}
		
		[Test]
		public function nextUntilOfLastIsEmpty():void
		{
			assertThat($(_child4).nextUntil(Point).length, equalTo(0));
		}
		
		[Test]
		public function nextUntilOfFirstIsAll():void
		{
			assertThat($(_child1).nextUntil(Point).toArray(), equalTo([_child2, _child3, _child4]));
		}
		
		[Test]
		public function filterOutSome():void
		{
			assertThat($(_child1).nextUntil(Sprite).toArray(), equalTo([_child2, _child3]));
		}
		
		[Test]
		public function filterOutAll():void
		{
			assertThat($(_child1).nextUntil(DisplayObject).length, equalTo(0));
		}
